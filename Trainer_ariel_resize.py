            ...
            for idx in pbar:
                ...
                if epoch > self.config.warmup_period:
                    ...
                    if mix_switch_1 == True:
                        ps_label_1_resize = F.resize(img=ps_label_1, size=(100, 100), interpolation=T.InterpolationMode.NEAREST)
                        nclasses_resize, counts  = torch.unique(ps_label_1_resize, return_counts=True)

                        if nclasses_resize.shape[0] > 2:
                            for x in range(torch.numel(counts)):
                    ...
                    if mix_switch_2 == True:
                        ps_label_2_resize = F.resize(img=ps_label_2, size=(100, 100), interpolation=T.InterpolationMode.NEAREST)
                        nclasses_resize, counts  = torch.unique(ps_label_2_resize, return_counts=True)
                    ...
                    if store < 10 and mix_switch_1 == True:
                        ...
                        #print(unsup_imgs_mixed_store)
                        #unsup_imgs_mixed_store = unsup_imgs_mixed_store 
                        #unsup_imgs_mixed_store = np.transpose(unsup_imgs_mixed_store, (1, 2, 0))  
                        #print(unsup_imgs_mixed_store.shape) 
                    ...
                    if store < 10 and mix_switch_2 == True:

                        name_0 = unsup_minibatch_0[1]
                        name_1 = unsup_minibatch_1[1]

                        ps_label_2_visualize = visualize(ps_label_2, mold_mapping)
                        ps_label_2_numpy = ps_label_2_visualize.cpu().numpy()
                        ps_label_2_save = ps_label_2_numpy.transpose(1,2,0)
                        ...
                        cv2.imwrite(self.log_dir + "/" + "ep" + str(epoch) + "_" + str(name_0[0][96:-4]) + "+" + str(name_1[0][96:-4]) + "_mask.png" , masks_save)
                        cv2.imwrite(self.log_dir + "/" + "ep" + str(epoch) + "_" + str(name_0[0][96:-4]) + "+" + str(name_1[0][96:-4]) + "_2.png" , ps_label_2_save)
                        cv2.imwrite(self.log_dir + "/" + "ep" + str(epoch) + "_" + str(name_0[0][96:-4]) + "+" + str(name_1[0][96:-4]) + "_img.png" , unsup_imgs_mixed_store)
                        store += 1
