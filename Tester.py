    ...
    def eval_and_predict(self):
        ...
        with torch.no_grad():
            ...
            for steps, (imgs, gts, filenames) in tqdm(enumerate(self.test_data_loader, start=1)):
                ...
                name = "".join(filenames) 
                #print(name[96:-4])
                pred_reshape = torch.reshape(pred, (1000, 1000)).cpu()
                #print(pred_reshape)
                pred_numpy = pred_reshape.numpy()
                cv2.imwrite(self.test_log_path + "/" +  name[96:-4] + ".png", pred_numpy)
                #print(self.test_log_path + "/" +  name[96:-4] + ".png")
                #write_png(input=pred_reshape, filename= self.test_log_path + "/" +  name[96:-4] + '.png')
